...
contract PendleAccessController is IPAccessManagerCore, AccessControlEnumerableUpgradeable, PendleRolesConstants {
    mapping(address target => mapping(bytes4 selector => bytes32 roles)) private __unused__AllowedRoles;
    mapping(address target => mapping(bytes4 selector => mapping(address caller => bool))) public allowedAddresses;
    mapping(bytes4 selector => mapping(address caller => bool)) public allowedSelectors;
    ...
    function setAllowedSelector(AllowedSelectorRequest[] calldata requests) external onlyRole(DEFAULT_ADMIN_ROLE) {
        for (uint256 i = 0; i < requests.length; i++) {
            AllowedSelectorRequest calldata req = requests[i];
            allowedSelectors[req.selector][req.caller] = req.allowed;
            emit AllowedSelectorSet(req.selector, req.caller, req.allowed);
        }
    }

    function canCall(address caller, address target, bytes4 selector) external view returns (bool) {
        return
            allowedSelectors[selector][caller] ||
            allowedAddresses[target][selector][caller] ||
