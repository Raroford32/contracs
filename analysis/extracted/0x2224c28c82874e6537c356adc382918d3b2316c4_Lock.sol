// Contract: Lock (0x2224c28c82874e6537c356adc382918d3b2316c4)
// Multi-file source
...
interface ILock {
    ...
    function unlockLiquidity(address _token) external returns (uint256);

    function unlockToken(address _token) external returns (uint256);
...
contract Lock is ILock {
    ...
    function unlockLiquidity(address _token) external override returns (uint256 _amount) {
        for (uint i = 0; i < liquidityList[_token].length; i) {
            if (
            ...
            }else if(liquidityList[_token][i].amount == 0){
                liquidityList[_token][i] = liquidityList[_token][
                    liquidityList[_token].length - 1
                ];
                liquidityList[_token].pop();
    ...
    function unlockToken(address _token) external override returns (uint256 _amount) {
        for (uint i = 0; i < tokenList[_token].length; i) {
            if (
            ...
            }else if(tokenList[_token][i].amount == 0){
                tokenList[_token][i] = tokenList[_token][tokenList[_token].length - 1];
                tokenList[_token].pop();
            }else{
                i = i + 1;
