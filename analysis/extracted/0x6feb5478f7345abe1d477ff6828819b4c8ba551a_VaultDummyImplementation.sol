// Contract: VaultDummyImplementation (0x6feb5478f7345abe1d477ff6828819b4c8ba551a)
// Multi-file source


// ===== FILE: contracts/dummyImplementations/InstaliteVaultV2.sol =====
...
contract ILeverageDexModule {
    /// @notice Core function to perform DEX refinance.
    /// @dev Note Flashloan will always be taken in `WSTETH`.
    /// @param protocolId_ Id of the protocol to refinance from.
    ...
    /// @notice Emitted whenever rebalancer refinances to fluid dex
    event LogLeverageDexRefinance(
        uint8 indexed protocolFrom,
        uint256 indexed route,
        uint256 wstETHflashAmount,
        uint256 wETHBorrowAmount,
...
contract IUnwindDexModule {
    /// @notice Core function to perform DEX Unwind refinance.
    /// @dev Note Flashloan will always be taken in `WSTETH`.
    /// @param protocolId_ Id of the protocol to refinance from.
    /// @param route_ Route for flashloan.
    /// @param wstETHflashAmount_ Amount of flashloan.
    /// @param wETHPaybackAmount_ Amount of wETH to be borrowed and paid back in source protocol.
    /// @param withdrawAmount_ Amount to be withdrawn. Will always be in stETH.
    /// @param perfectColShares_ The change in collateral shares (positive for deposit, negative for withdrawal)
    /// @param colToken0MinMax_ Min or max collateral amount of token0 to withdraw or deposit (positive for deposit, negative for withdrawal)
    /// @param colToken1MinMax_ Min or max collateral amount of token1 to withdraw or deposit (positive for deposit, negative for withdrawal)
    ...
    /// @notice Emitted whenever rebalancer refinances to fluid dex
    event LogUnwindDexRefinance(
        uint8 indexed protocolFrom,
        uint256 indexed route,
        uint256 wstETHflashAmount,
        uint256 wETHPaybackAmount,
        uint256 withdrawAmount,
        int256 perfectColShares,
        int256 colToken0MinMax,
        int256 colToken1MinMax,
...
contract IRebalancerModule {
    ...
    /// @notice Swaps KING tokens to WETH.
    /// @param sellAmount_ Amount of KING to sell.
    /// @param unitAmount_ Amount of WETH to buy.
    /// @param swapConnectorName_ Name of the connector to use for the swap.
    /// @param swapCallData_ Call data for the swap.
    ...
    /**
     * @notice Open function to sweep ideal `weth` to `weeth` in Dsa.
     */
    function sweepWethToWeEth() public {}

    /// @notice Transfers KING tokens to Team MS.
    /// @param amount_ Amount of KING to transfer.
    function transferKingTokensToTeamMS(uint256 amount_) public {}
    ...
    /// @notice Emitted whenever ideal Weth DSA balance is swept to WEETH.
    event LogWethSweepToWeEth(uint256 wethAmount);

    /// @notice Emitted whenever King rewards are swapped.
    event LogSwapKingTokensToWeth(
        uint256 sellAmount,
        uint256 unitAmount,
        string swapConnectorName
    );
    ...
    /// @notice Emitted whenever King tokens are transferred to Team MS.
    event LogTransferKingTokensToTeamMS(
...
contract ILeverageModule {
     ...
     * @param swapMode_ Mode of swap.(0 = no swap, 1 = Swap Aggregator, 2 = direct Lido route)
     * @param _swapConnectors Array of swap aggregator connectors to route swap from.
     * @param _swapDatas Array of bytes calldata required for `WETH` to `WSTETH` swapping.
...
contract IWithdrawalsModule {
    ...
    function paybackDebt(uint8 toProtocolId_) external {}
...
contract IAdminModule {
    /// @notice Sets DSA's Fluid DEX NFT ID.
    function setFluidDexNftId(uint256 nftID_) external {}

    /// Sets Aave V3 Lido WETH Correlated E Mode.
    function enableAaveV3LidoEMode() public {}
...
contract IViewModule {
    ...
    struct ProtocolAssetsAaveV3 {
        uint256 wstETH; // supply
        uint256 weETH; // supply
        uint256 eETH; // weETH supply converted to eETH
        uint256 stETH; // wstETH supply converted to stETH
        uint256 wETH; // borrow
    }

    struct ProtocolAssetsFluidWeETH {
        uint256 weETH; // supply
        uint256 wstETH; // borrow
        uint256 eETH; // weETH supply converted to eETH
        uint256 stETH; // wstETH borrow converted to stETH
    }

    struct ProtocolAssetsInDexWstETH {
        uint256 wstETHSupply;
        uint256 ethSupply;
        uint256 wstETHBorrow;
        uint256 stETHBorrow;
        uint256 ethBorrow;
    }
    ...
    struct NetAssetsHelper {
        ...
        ProtocolAssetsInDexWstETH fluidDex;
        ProtocolAssetsFluidWeETH fluidWeETH;
    ...
    /// Returns ratio of Aave V3 in terms of `WETH` and `(STETH + EETH)`.
    ...
    /// @param eEthPerWeETH_ Amount of eETH for one weETH.
    /// @param stEthPerWsteth_ Amount of stETH for one wstETH.
    /// `stEthPerWsteth_` can be sent as 0 and it will internally calculate the conversion rate.
    /// `eEthPerWeETH_` can be sent as 0 and it will internally calculate the conversion rate.
    /// This is done to save on gas by removing conversion rate calculation for each protocol.
    /// Returns ratio of Fluid in terms of `eETH` and `STETH`.
...
contract IVariablesRead {
    ...
    /// @notice DSA's NFT ID for fluid vault T4.
    function fluidDexNFT() public view returns (uint256) {}

    /// @notice max allocation to team multisig - in wstETH (for future use)
    function maxAllocationToTeamMultisig() public view returns (uint256) {}

    /// @notice allocation to team multisig - in wstETH
    function allocationToTeamMultisig() public view returns (uint256) {}
}

contract IFluidStethModule {
    /// @notice Core function to perform steth queueing refinance.
    /// @dev Note Flashloan will always be taken in `WETH` here.
    /// @param fromProtocolId_ Id of the protocol to refinance from.
    /// @param route_ Route for flashloan.
    /// @param wETHBorrowAmount_ Amount of wETH for flashloan and to be borrowed.
    /// @param withdrawStethAmount_ Amount of stETH to be withdrawn. Will always be in stETH.
    function queueSteth(
        uint8 fromProtocolId_,
        uint256 route_,
        uint256 wETHBorrowAmount_,
        uint256 withdrawStethAmount_
    ) external returns (uint256 ratioFromProtocol_) {}

    /// @notice Core function to claim ETH from stETH request.
    /// @dev Note After claiming ETH, it will be converted to WETH and sent to the vault.
    /// @param fromNftID_ NFT Id from which we have to start claiming.
    function claimSteth(
        uint256 fromNftID_
    ) external returns (uint256 claimed_) {}

    /// @notice Core function to claim ETH and Payback debt in fluid.
    /// @param fromNftID_ NFT Id from which we have to start claiming.
    function claimStethAndPaybackFluid(
        uint256 fromNftID_
    ) external returns (uint256 ethClaimed_) {}

    /// @notice Emitted when stETH goes for queueing.
    event LogQueueSteth(
    ...
    /// @notice Emitted when stETH is claimed from stETH protocol.
    event LogClaimSteth(uint256 fromNFTId, uint256 ethClaimed);

    /// @notice Emitted when stETH is claimed and eth debt is paid back in fluid.
    event LogClaimStethAndPaybackFluid(uint256 fromNFTId, uint256 ethClaimed);
}

contract IClaimModule {
    /// @notice Claims rewards from Spark.
    function claimFromSpark() external {}

    /// @notice Claims rewards from Aave V3 Lido.
    function claimFromAaveV3Lido() external {}

    /// @notice Claims rewards from King.
    /// @param merkleContract_ Address of the merkle contract.
    /// @param amount_ Amount of rewards to claim.
    ...
    // Emitted when rewards are claimed from Spark.
    event LogClaimFromSpark(address[] poolAddresses);

    // Emitted when rewards are claimed from Spark.
    event LogClaimFromAaveV3Lido(address[] rewardAddresses);

    // Emitted when rewards are claimed from King.
    event LogClaimFromKing(uint256 amount);
}

contract IStethToEethModule {
    ...
    event LogConvertAaveV3wstETHToWeETH(
        uint256 wEthFlashloanAmount,
        uint256 wstEthWithdrawAmount,
        uint256 route
    );
}

contract IFluidAaveV3WeETHRebalancerModule {
    ...
    event LogRebalanceFromWstETHToWeETH(
        uint256 indexed wstEthFlashloanAmount_,
        uint256 indexed weEthWithdrawAmount_,
        uint256 indexed route_
    );

    event LogRebalanceFromWeETHToWstETH(
        uint256 indexed weEthFlashloanAmount_,
        uint256 indexed wstEthBorrowAmount_,
        uint256 indexed route_
    );
}

contract IAaveV3WstETHWeETHSwapModule {
    ...
    event LogSwapWstETHToWeETH(
        uint256 indexed wstEthSellAmount_,
        uint256 indexed route_,
        uint256 revenueEarned_,
        string swapConnectorName_
    );

    event LogSwapWeETHToWstETH(
        uint256 indexed weEthSellAmount_,
        uint256 indexed route_,
        uint256 revenueEarned_,
        string swapConnectorName_
    );
...
contract VaultDummyImplementation is
    ...
    IUnwindDexModule,
    IStethToEethModule,
    IFluidAaveV3WeETHRebalancerModule,
    IAaveV3WstETHWeETHSwapModule
{
