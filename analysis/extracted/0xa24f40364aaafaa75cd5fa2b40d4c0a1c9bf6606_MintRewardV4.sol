// Contract: MintRewardV4 (0xa24f40364aaafaa75cd5fa2b40d4c0a1c9bf6606)
// Multi-file source
...
// ===== FILE: contracts/IGoMiningToken.sol =====
// SPDX-License-Identifier: MIT
...
// ===== FILE: contracts/IMintReward.sol =====
// SPDX-License-Identifier: MIT
pragma solidity 0.8.21;
...
// ===== FILE: contracts/IRewardConfig.sol =====
// SPDX-License-Identifier: MIT
pragma solidity 0.8.21;

interface IRewardConfig {
    function autoClaimers(address) external view returns (bool);
...
// ===== FILE: contracts/IVEGoMiningToken.sol =====
// SPDX-License-Identifier: MIT
pragma solidity 0.8.21;
...
// ===== FILE: contracts/MintRewardV4.sol =====
// SPDX-License-Identifier: MIT
pragma solidity 0.8.21;
...
contract MintRewardV4 is IMintReward, PausableUpgradeable, AccessControlUpgradeable, ReentrancyGuardUpgradeable, UUPSUpgradeable {
    ...
    function unclaimedRewards(address _addr) public view returns (uint256) {
        if (RewardConfig.autoClaimers(_addr)) {
            return 0;
        }
        uint256 rc = rewardCount;
    ...
    // @notice Returns calculated amount of unclaimed rewards for `msg.sender` related to lastRewardIndex
    function unclaimedRewards(address _addr, uint256 lastRewardIndex) external view returns (uint256) {
        uint256 rc = rewardCount;
    ...
    function rewardHistory(address _addr) external view returns (uint256[] memory) {
        uint256 rc = rewardCount;

        IVEGoMiningToken _veToken = veToken;
        uint256[] memory values = new uint256[](rc);
        ...
        for (uint256 i = 1; i < rc; i++) {
            ...
            values[i] = balance * reward.amount / reward.ts;
        }
    ...
    function claimRewards() external nonReentrant whenNotPaused {
        ...
        require(!RewardConfig.autoClaimers(_msgSender()), "MintReward: not allowed to claim rewards");
    ...
    function updateForV4(address _rewardConfig) external onlyRole(UPGRADER_ROLE) {
        RewardConfig = IRewardConfig(_rewardConfig);
    }
    ...
    function setLastClaim(address _addr, uint256 lastClaimIndex) external onlyRole(UPGRADER_ROLE) {
        require(lastClaimIndex > 0 && lastClaimIndex < rewardCount, "MintReward: index is out of range");

        lastClaim[_addr] = lastClaimIndex;
    }

    function withdraw(uint256 amount) external onlyRole(CONFIGURATOR_ROLE) {
        require(Token.balanceOf(address(this)) > amount, "MintReward: no rewards");

        require(Token.transfer(_msgSender(), amount), "MintReward: token transfer failed");
    }
