// Contract: stakingManagerV1 (0x0ce62a94dbc2335b227496b61e853880c5dbcd67)
// Multi-file source
...
contract stakingManagerV1 is OwnableUpgradeable {
  ...
  function __stakingManager_init(address _stakeTokenAddress, address _presaleContract, uint256 _lockTime, uint256 _apy, uint256 _endTime, uint256 _maxDepositLimit, uint256 _withdrawLockedTime) public initializer {
    __Ownable_init_unchained();
    stakeToken = IERC20Upgradeable(_stakeTokenAddress);
    rewardsPrecision = 10 ** 18;
  ...
  function deposit(uint256 _amount) external {
    ...
    require(tokensStaked + _amount <= maxDepositLimit, 'Deposit amount reached');
  ...
  function _harvestRewards(address _user) private {
    ...
    uint256 rewards = ((timePassedRatio * apy * staker.amount) / rewardsPrecision) / 100;
  ...
  function withdraw() external {
    require(!isBlacklisted[_msgSender()], 'This Address is Blacklisted');
    require(!withdrawLocked, 'Withdraw are Locked');
