// Contract: DecubateVestingV2 (0x2375fc81443fc333ae99574ce77da10ce4baed22)
// Multi-file source
...
// ===== FILE: src/DecubateVestingV2.sol =====
// SPDX-License-Identifier: MIT

//** Decubate vesting Contract */
//** Author Aceson : Decubate Vesting Contract 2024.5 */
...
contract DecubateVestingV2 is AccessControlUpgradeable, DateTime {
    enum Type {
        Linear,
        Monthly,
        Interval
    ...
    modifier notDisabled(uint256 _option) {
        require(!vestingPools[_option].isDisabled, "Strategy is disabled");
        _;
    }
    ...
    function initialize(ClaimFee memory claim, address _token, address _admin) external initializer {
        require(claim.claimFeePercent <= (DIVISOR * 10) / 2, "Invalid fee");
    ...
    function addVestingStrategy(
        ...
        returns (bool)
    {
        require(_start > 0, "Invalid time");
        require(_initialUnlockPercent <= DIVISOR, "Invalid unlock percent");
        ...
        if (_type == Type.Interval) {
            ...
            for (uint16 i = 0; i <= DIVISOR; i += _unlockPerInterval) {
                month += _monthGap;
    ...
    function setVestingStatus(
        ...
        returns (bool)
    {
        vestingPools[_option].isDisabled = _status;
        return true;
    }

    function setMaxTokenTransfer(uint256 _amount, bool _active) external onlyManager returns (bool) {
        maxTokenTransfer.amount = _amount;
        maxTokenTransfer.active = _active;
        return true;
    }

    function setToken(address _addr) external onlyManager returns (bool) {
        require(_addr != address(0), "Invalid address");
        token = IERC20Upgradeable(_addr);
        return true;
    }

    function setClaimFee(ClaimFee memory _claim) external {
        require(msg.sender == claimFee.claimFeeReceiver, "Restricted");
        require(_claim.claimFeePercent <= (DIVISOR * 10) / 2, "Invalid fee");
        claimFee = _claim;
    }
    ...
    function revoke(uint256 _option, address _wallet) external onlyManager userInWhitelist(_option, _wallet) {
        uint256 idx = vestingPools[_option].hasWhitelist[_wallet].arrIdx;
    ...
    function transferToken(address _addr, uint256 _amount) external onlyManager returns (bool) {
    ...
    function claimDistribution(uint256 _option, address _wallet) public notDisabled(_option) returns (bool) {
        uint256 idx = vestingPools[_option].hasWhitelist[_wallet].arrIdx;
        WhitelistInfo storage whitelist = vestingPools[_option].whitelistPool[idx];

        require(!whitelist.disabled, "User is disabled from claiming");
        ...
        if (claimFee.claimFeePercent > 0) {
            uint256 fee = (releaseAmount * claimFee.claimFeePercent) / (DIVISOR * 10); //Extra 0
            token.transfer(claimFee.claimFeeReceiver, fee);
            emit FeeClaimed(claimFee.claimFeeReceiver, fee);
        }
    ...
    function getVestingInfo(uint256 _strategy) public view optionExists(_strategy) returns (VestingInfo memory) {
        return VestingInfo({
            ...
            isDisabled: vestingPools[_strategy].isDisabled,
        ...
        returns (uint256 amount)
    {
        ...
        // initial unlock
        uint256 initial = (whitelist.dcbAmount * vest.initialUnlockPercent) / DIVISOR;
        ...
        returns (uint256)
    {
        ...
        uint256 remaining = whitelist.dcbAmount - initial;
        ...
        returns (uint256)
    {
        ...
        uint256 intervalsPassed = (block.timestamp - vest.cliff) / vest.interval;
        uint256 totalUnlocked = intervalsPassed * vest.unlockPerInterval;

        if (totalUnlocked >= DIVISOR) {
            return whitelist.dcbAmount;
        } else {
            return initial + (remaining * totalUnlocked) / DIVISOR;
        }
    }
...
// ===== FILE: src/libraries/DateTime.sol =====
