// Contract: CampaignRewardDistribution (0x717d0bf97ce58e14945f5e0320ee98381aeaddaf)
// Multi-file source
...
// ===== FILE: contracts/CampaignRewardDistribution.sol =====
...
contract CampaignRewardDistribution is OwnableUpgradeable {
    ...
    function updateEnabled(uint256 _campaignId, bool _enabled) external onlyOwner {
        campaigns[_campaignId].enabled = _enabled;
    }

    function updateRewardToken(uint256 _campaignId, address _rewardToken) external onlyOwner {
        campaigns[_campaignId].rewardToken = _rewardToken;
    }
    
    function updateClaimableFrom(uint256 _campaignId, uint256 _claimableFrom) external onlyOwner {
        require(_claimableFrom < campaigns[_campaignId].claimableUntil, "claimableFrom must be before claimableUntil");
        campaigns[_campaignId].claimableFrom = _claimableFrom;
    }

    function updateClaimableUntil(uint256 _campaignId, uint256 _claimableUntil) external onlyOwner {
        require(_claimableUntil > campaigns[_campaignId].claimableFrom, "claimableUntil must be after claimableFrom");
        campaigns[_campaignId].claimableUntil = _claimableUntil;
    }
    
    function updateSignaturePrefix(uint256 _campaignId, string memory _signaturePrefix) external onlyOwner {
        campaigns[_campaignId].signaturePrefix = _signaturePrefix;
    }
    ...
    function createCampaign(uint256 _campaignId, address _rewardToken, uint256 _claimableFrom, uint256 _claimableUntil, string memory _signaturePrefix) external onlyOwner {
        require(_claimableFrom < _claimableUntil, "claimableFrom must be before claimableUntil");
        campaigns[_campaignId] = Campaign(true, _rewardToken, _claimableFrom, _claimableUntil, _signaturePrefix);
        emit CampaignCreated(_campaignId, _rewardToken);
    }

    function claim(address user, uint256 campaignId, uint256 amount, bytes memory signature) external {
        require(campaigns[campaignId].enabled, "campaign not enabled");
